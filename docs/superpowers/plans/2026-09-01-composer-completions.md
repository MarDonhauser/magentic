...
export function completionTrigger(text, caret) {
  for (let i = caret - 1; i >= 0; i--) {
    ...
    // deutet ihn auch nur dort als Befehl.
...
// applyCompletion ersetzt den Auslöserbereich durch den gewählten Wert und
// hängt ein Leerzeichen an, damit weitergetippt werden kann. Der Text hinter
// der Schreibmarke bleibt unangetastet.
export function applyCompletion(text, trigger, value) {
  ...
  return {
    text: `${head}${inserted}${separator}${tail}`,
    caret: head.length + inserted.length + separator.length,
  };
}
```
